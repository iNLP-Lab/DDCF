...
def convert_pow(expr: str) -> str:
    # 1) Handle the brace form: 10^{n} → 10**n
    expr = re.sub(r'10\^\{([^}]+)\}', r'10**\1', expr)
    # 2) Then handle the un-braced form: 10^n → 10**n
    expr = re.sub(r'10\^([A-Za-z0-9_]+)', r'10**\1', expr)
    return expr
...
def normalize_final_answer(final_answer: str) -> str:
    ...
    final_answer = convert_pow(final_answer)
    ...
    # final_answer = final_answer.replace("°", "^\\circ")
...
def clean_entry(s: str) -> str:
    if s.find("\\text{") == -1:
        return s
    """
    Normalize a single answer string by:
     1. Unwrapping \text{…}, \(…\), and parentheses.
     2. Removing any stray backslashes.
     3. Splitting on commas, stripping whitespace.
     4. Sorting the items for a canonical, comma-joined output.
    """
    # 1. Unwrap \text{…}
    s = re.sub(r'\\text\{([^}]*)\}', r'\1', s)
    # 2. Remove LaTeX delimiters \( and \), and any remaining backslashes
    s = re.sub(r'\\\(|\\\)', '', s)
    s = s.replace('\\', '')
    # 3. Unwrap plain parentheses if they enclose the whole string
    s = s.strip()
...
def math_equal(
    ...
    prediction = clean_entry(prediction)
    reference = clean_entry(reference)
    prediction = normalize_final_answer(prediction)
    reference = normalize_final_answer(reference)

    if (prediction.find("\\left") != -1) and (prediction.find("\\right") != -1):
        prediction = prediction.replace("\\left", "")
        prediction = prediction.replace("\\right", "")
    if (reference.find("\\left") != -1) and (reference.find("\\right") != -1):
        reference = reference.replace("\\left", "")
        reference = reference.replace("\\right", "")
...
def allowed_error(num) -> Decimal:
    """
    Given a number (as str or numeric), returns the place‐value of its
    least significant digit, e.g.:
      1.1    → Decimal('0.1')
      1.0001 → Decimal('0.0001')
      42     → Decimal('1')
    """
    d = Decimal(str(num))
    # d.as_tuple().exponent is negative # of decimal places
    return Decimal(10) ** d.as_tuple().exponent
...
#     # fix rounding errors
#     error = allowed_error(reference)*0.5
#     return (isclose(reference, prediction, abs_tol=error)) or (isclose(reference, prediction, rel_tol=1e-4))
...
def numeric_equal(prediction: float, reference: float):
    ...
    # else:
    # prediction = round(prediction, len(str(reference).split(".")[-1]))
    return isclose(reference, prediction, rel_tol=1e-2)
